...
export const checkVulnLines = () => async (req: Request<Record<string, unknown>, Record<string, unknown>, VerdictRequestBody>, res: Response, next: NextFunction) => {
  ...
  const infoFilePath = path.resolve(codefixesDir, key + '.info.yml')
  // Validate that the resolved path is within the codefixes directory
  if (infoFilePath.startsWith(codefixesDir) && fs.existsSync(infoFilePath)) {
